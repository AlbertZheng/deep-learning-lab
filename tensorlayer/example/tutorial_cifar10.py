...
def model(x, y_, reuse):
    ...
    with tf.variable_scope("model", reuse=reuse):
        ...
        # 这里LRN层放在池化层之后跟下面代码里的放在池化层之前，有什么差别呢？
...
x_batch = tf.placeholder(dtype=tf.float32, shape=[None, 24, 24, 3], name='x')
#x_batch = tf.placeholder(dtype=tf.float32, shape=[None, 32, 32, 3], name='x')
y_label_batch = tf.placeholder(dtype=tf.int64, shape=[None], name='y_')
...
network, cost, _ = model_batch_norm(x_batch, y_label_batch, False, is_train=True)
_, cost_test, acc = model_batch_norm(x_batch, y_label_batch, True, is_train=False)
...
for epoch in range(n_epoch):
    ...
    for X_train_a, y_train_a in tl.iterate.minibatches(X_train, y_train, batch_size, shuffle=True):
        """这个threading_data(...,fn=distort_fn,...)调用太慢了，取消这里的data augmentation之后，
        GPU加速环境下训练速度从原来的 >44s 缩短到 <22s 。看来python对于计算密集型应用还是偏弱，数据ETL
        之类的应用还是得Java或C++来干才合适。
        """
        X_train_a = tl.prepro.threading_data(X_train_a, fn=distort_fn, is_train=True)  # data augmentation for training
        #X_train_a = tl.prepro.threading_data(X_train_a, fn=dummy_fn, is_train=True)
        sess.run(train_op, feed_dict={x_batch: X_train_a, y_label_batch: y_train_a})
    ...
    if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
        ...
        for X_test_a, y_test_a in tl.iterate.minibatches(X_test, y_test, batch_size, shuffle=False):
            ...
            #X_test_a = tl.prepro.threading_data(X_test_a, fn=dummy_fn, is_train=False)
            err, ac = sess.run([cost_test, acc], feed_dict={x_batch: X_test_a, y_label_batch: y_test_a})
            test_loss += err
            test_acc += ac
            n_batch += 1
