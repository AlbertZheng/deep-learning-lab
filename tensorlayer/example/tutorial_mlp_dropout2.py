...
def mlp(x, is_train=True, reuse=False):
    with tf.variable_scope("MLP", reuse=reuse):
        ...
        # is_fix – Fixing probability or nor. Default is False. If True, the keeping probability is fixed and cannot be changed via `feed_dict`.
        # is_train – Trainable or not. If False, skip this layer. Default is True.
        # 我们使用is_fix=True，如此keep值将被fix住、将不能被feed_dict进行修改
        # 当在tesing时，我们将传入is_train=False，如此将使得这个dropout层被skip掉，即没有dropout
...
train_params = tl.layers.get_variables_with_name('MLP', True, True)
